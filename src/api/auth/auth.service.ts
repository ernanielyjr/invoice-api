...
import { ResponseError, ErrorMessages, httpStatus } from '../../models/response.model';
    ...
    if (token) {
      jwt.verify(token, AppConfig.jwt.secret, (err, decoded) => {
        if (err) {
          ...
          new ResponseError(res, ErrorMessages.AUTH_INVALID_TOKEN, httpStatus.UNAUTHORIZED);
        } else {
          next();
        }
      });
      ...
      new ResponseError(res, ErrorMessages.AUTH_MISSING_TOKEN, httpStatus.FORBIDDEN);
    }
  }
